...
if (!defined('MW_API_URL')) {
    ...
    define('MW_API_URL', $env_api_url ?: 'http://api:3000/api/v1');
}
...
function mw_api_get($endpoint, $params = []) {
    ...
    if (is_wp_error($response)) {
        ...
        error_log(sprintf('[MW_API_ERROR] GET %s | Network Error: %s | Duration: %.2f ms', $endpoint, $err_msg, $duration_ms));
        return null;
    }
    $code = wp_remote_retrieve_response_code($response);
    if ($code !== 200) {
        if ($code !== 404) {
            error_log(sprintf('[MW_API_ERROR] GET %s | HTTP Code: %d | Duration: %.2f ms', $endpoint, $code, $duration_ms));
        }
        return null;
    }
    ...
    if (!is_array($data)) {
        error_log(sprintf('[MW_API_ERROR] GET %s | Invalid JSON Payload | Duration: %.2f ms', $endpoint, $duration_ms));
        return null;
    }
    return $data;
}
