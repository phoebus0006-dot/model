...
export async function buildApp(options?: BuildAppOptions) {
  ...
  if (!options?.skipLifecycle) {
    ...
    app.addHook("onClose", async () => {
      ...
      try { await redis.quit(); } catch { /* not connected */ }
    });
  }
  ...
  app.addHook("onRequest", async (req, reply) => {
    ...
    const isEntityWritePath =
      (p.startsWith("/api/v1/manufacturers") ||
        p.startsWith("/api/v1/series") ||
        p.startsWith("/api/v1/sculptors") ||
        p.startsWith("/api/v1/categories") ||
        p.startsWith("/api/v1/characters")) &&
      isWriteMethod;
